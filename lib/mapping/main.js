...
class RaidControl {
    ...
    static RaidEnemy = class {
        ...
        move() {
            ...
            let path_index  = 0;
            let speed_delay = 20 * (100 - this.speed);
        ...
        trackPath(startPosition) {
            ...
            const tileCartesian = (_tile) => {
                let _x = Number(_tile.getAttribute('x'));
                let _y = Number(_tile.getAttribute('y'));
                return {'x': _x, 'y': _y};
            };
            ...
            while(!castle_side && iter_limit--) {
                let possible_paths = {
                    'top'   : this.tableMap.findInstance(anchor.x, anchor.y+1),
                    'right' : this.tableMap.findInstance(anchor.x+1, anchor.y),
                    'bottom': this.tableMap.findInstance(anchor.x, anchor.y-1),
                    'left'  : this.tableMap.findInstance(anchor.x-1, anchor.y)
                };
                let counter_side = {
                    'top'   : "bottom",
                    'right' : "left",
                ...
                let valid_paths = [];
                for(let _path of Object.keys(possible_paths)) {
                    if(possible_paths[_path]) {
                        let cartesian = tileCartesian(possible_paths[_path]);
                        let is_path = this.tableMap.Helper.isPath(this.tableMap.mapping[cartesian.y][cartesian.x]);
                        if(this.tableMap.Helper.isCastle(this.tableMap.mapping[cartesian.y][cartesian.x])) {
                            valid_paths = [_path];
                            break;
                        }
                        else if(is_path && (last_path === null || last_path !== _path)) {
                            valid_paths.push(_path);
                        }
                    }
                }
                
                let direction = valid_paths[Math.round(Math.random()*(valid_paths.length-1))];
                if(direction) {
                    last_path = counter_side[direction];
                    anchor    = tileCartesian(possible_paths[direction]);
    ...
    get orc() {
        return new RaidControl.RaidEnemy("Orc", RaidControl.ICON_PATH+"orc.png", 5, 200, 5);
    }
    get wolf() {
        return new RaidControl.RaidEnemy("Wolf", RaidControl.ICON_PATH+"wolf.png", 20, 80, 2);
    }
    get bat() {
    ...
    nextWave() {
        ...
        for(let spawned=0; spawned < spawnAmount; spawned++) {
            this.summonEnemy(this.goblin, getRandomSpawner(), spawned);
        }
    }
