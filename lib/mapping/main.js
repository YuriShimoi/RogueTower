class TableMap {
    ...
    plot(_element=this.element) {
        ...
        for(let _y in this.mapping) {
            ...
            for(let _x in this.mapping[_y]) {
                ...
                if(isPath(_tile)) {
                    if(_tile.spawn) tileHTML.classList.add("tbm-spawn");
        ...
        _element.appendChild(tableHTML);
    }
    ...
    generateMap() {
        ...
        const buildPath = (anchor, xMoves, yMoves, split_chance=0.8, distance=0) => {
            ...
            while(isValidPos(anchor.x, anchor.y) && !pathJoin(anchor, last_move, ignore_first)) {
                ignore_first = false;
                let moveOn = "";
            ...
            if(!isValidPos(anchor.x, anchor.y) && last_move) {
                this.mapping[anchor.y+direction_info[last_move].counter.y][anchor.x+direction_info[last_move].counter.x].spawn = true;
            }
        };
