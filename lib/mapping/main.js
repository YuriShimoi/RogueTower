...
class RaidControl {
    ...
    static RaidEnemy = class {
        ...
        trackPath(startPosition, bestPath=true) {
            ...
            if(bestPath) {
                ...
                let best_path = trackBest(startPosition.x, startPosition.y);
                this.path = [...this.path, ...best_path.path];
            }
            // Random Path
            else {
    ...
    nextWave() {
        ...
        for(let spawned=0; spawned < spawnAmount; spawned++) {
            this.summonEnemy([this.goblin, this.wolf, this.orc, this.bat][Math.round(Math.random()*3)], getRandomSpawner(), spawned);
        }
    }
